...
export async function uploadToS3(zipFilePath: string, companyId: string): Promise<void> {
    logger.info(`Iniciando o upload para o S3 para a empresa: ${companyId}`);
    ...
    logger.info(`Preparando para enviar arquivo para o bucket ${bucketName} com chave ${key}`);
    ...
    try {
      ...
      logger.info(`Arquivo enviado com sucesso para o MinIO no caminho: backups/${companyId}/${timestamp}_${path.basename(zipFilePath)}`);
    } catch (err: unknown) {
      if (err instanceof Error) {
        logger.error(`Erro ao enviar arquivo para o MinIO: ${err.message}`);
      } else {
        logger.error('Erro ao enviar arquivo para o MinIO: Erro desconhecido');
      }
    }
}
